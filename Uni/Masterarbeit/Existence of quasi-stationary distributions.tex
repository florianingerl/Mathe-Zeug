...
\begin{proof}
"$\Leftarrow$" is trivial. "$\Rightarrow$": In this proof, we make repeated use of the Monotone Class Theorem, see Theorem \ref{th:MonotoneClass}. With the notation chosen as in this theorem, we define the sets
...
\end{align*}
$\mathcal{H}$ satisfies the condition of the Monotone Class Theorem, because of linearity of the conditional expectation and because of the dominated convergence of the conditional expectation. Because $P$ is a $\cap$-stable generator of the $\sigma$-algebra $\mathcal{B}\left(E\right)^{\otimes \R_{\geq 0}}$ it suffices to prove for $A \in P$ that the indicator $\textbf{1}_A$ is in $\mathcal{H}$. For this, we show the following claim by induction over $n$:\\
For all $n \in \N$ for all $0 \leq t_1 \leq \ldots \leq t_n < \infty $, the set $\mathcal{H}$ contains all $\mathcal{B}^{t_1,\ldots,t_n}$-measurable bounded functions (where $\mathcal{B}^{t_1,\ldots,t_n}$ is the smallest $\sigma$-algebra on $E^{\R_{\geq 0}}$ such that the given projections in the exponent are measurable). For simplicity of notation, let $\Theta_s = \left(X\left(t+s\right)\right)_{t \in \R_{\geq 0}$.\\
\textbf{n=1}: So let $0 \leq t_1$ and $A_1 \in \mathcal{B}\left(E\right).$ Then for all $s\geq 0, x \in E$ holds because of the weak Markov property
\begin{align*}
...
\begin{definition}
We say that a Markov process $\left(X\left(t\right)\right)_{t \in \R_{\geq 0}}$ has the \textbf{strong Markov property} with respect to a stopping time $ \tau $, if we have a.s. on $\lbrace \tau < \infty \rbrace $
\begin{equation} \label{eq:strongMarkovproperty}
P_x\left(X\left(\tau+t\right) \in A | \mathcal{F}_{\tau} \right) = P_{X\left(\tau\right)}\left(X\left(t\right) \in A \right)
\end{equation}
...
%\textbf{Example: Linear birth and death process}
%A birth and death process with birth and death coefficents is defined somewhere else. Here we have $\mu_0 = \lambda_0 = 0$ and we are given $ 0 < \lambda < \mu $ and $ \lambda_n = n \lambda, \mu_n = n \mu $ to be the birth and death coefficients.
%
%Take $f\left(x\right) = x .$ Take $ 0 < D_1 < \frac{\mu - \lambda}{\lambda + \mu}.$ Then for all $ x \geq 1 $ holds:
%\begin{align*}
%\sum_{y\neq x} \frac{q\left(x,y\right)}{q\left(x\right)} f\left(y\right) &= \frac{q\left(x,x-1\right)}{q\left(x\right)} f\left(x-1\right) + \frac{q\left(x,x+1\right)}{q\left(x\right)} f\left(x+1\right) \\
%&= \frac{x \mu}{x \left(\lambda + \mu \right) } \left(x-1\right) + \frac{x \lambda }{x\left(\lambda + \mu \right)} \left(x+1\right) \\
%&= x + \frac{\lambda - \mu}{\lambda + \mu} \leq x - D_1.
%\end{align*}
%Take $D2 = 1$ , then for all $ x \geq 0 $ and all $ y $ with $ q\left(x,y\right) > 0 $ holds $\left|f\left(x\right) - f\left(y\right) \right| \leq 1 = D_2.$ This is some bounded jumps condition!!
%The other conditions are also easiliy verified, so this process has a quasi-stationary distribution
%
%\textbf{Example:}
...
is called a \textbf{birth and death process} on $\N_0$ with birth coefficients $\lambda_n$ and death coefficients $\mu_n, n \geq 0. $
\end{definition}
